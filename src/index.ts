...
export default class NxGate {
  private static readonly DEFAULT_SERVER = 'https://api.nxgate.noxlyde.xyz';

  private userId: string;
  private publicRsaKey?: string;
  private validationServer: string = NxGate.DEFAULT_SERVER;
  ...
  setPublicRsaKey(publicRsaKey: string): NxGate {
    this.publicRsaKey = publicRsaKey;
    return this;
  }

  setValidationServer(validationServer: string): NxGate {
    this.validationServer = validationServer;
    return this;
  }

  useChallenges(): NxGate {
    this.useChallenge = true;
    return this;
  }
  ...
  private async requestServer(url: string): Promise<any> {
    const response = await axios.get(url, { headers: { 'User-Agent': 'Bensonheimer992/NxGate-JS Wrapper' } });
    if (this.debugMode) {
